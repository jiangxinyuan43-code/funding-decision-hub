...
export function FinanceEditor({ open, onClose, notify }: Props) {
  ...
  useEffect(() => { if (open && storedPlan) { setPlan(storedPlan); setGoals(storedGoals) } }, [open, storedGoals, storedPlan])
  if (!plan) return null
  ...
  return (
    <Modal open={open} onClose={onClose} title="更新资金计划" description="这里是预测参数，不会自动修改你的实际余额。" size="wide">
      <div className="form-grid">
        ...
        <label><span>每月工资</span><div className="money-input"><i>¥</i><input type="number" inputMode="decimal" value={plan.monthlyIncome} onChange={(event) => number('monthlyIncome', event.target.value)} /></div></label>
        <label><span>每月固定支出</span><div className="money-input"><i>¥</i><input type="number" inputMode="decimal" value={plan.monthlyFixedExpense} onChange={(event) => number('monthlyFixedExpense', event.target.value)} /></div></label>
        <label><span>每月固定储蓄</span><div className="money-input"><i>¥</i><input type="number" inputMode="decimal" value={plan.monthlySaving} onChange={(event) => number('monthlySaving', event.target.value)} /></div></label>
        ...
        <label><span>一次性额外收入</span><div className="money-input"><i>¥</i><input type="number" inputMode="decimal" value={plan.extraIncome} onChange={(event) => number('extraIncome', event.target.value)} /></div></label>
